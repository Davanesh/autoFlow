...
export default function CanvasArea() {
  ...
  function startDrag(e, nodeId) {
    ...
    selected.forEach((id) => {
      ...
      if (!n) return;
      newOffsets[id] = {
        x: (e.clientX - rect.left - pan.x) / zoom - n.x,
        y: (e.clientY - rect.top - pan.y) / zoom - n.y,
      };
    });
    setOffsets(newOffsets);
  }

  // --- Move nodes while dragging
  function onMouseMove(e) {
    ...
    if (panning) {
      setPan({
        x: panStart.x + (e.clientX - panStart.startX),
        y: panStart.y + (e.clientY - panStart.startY),
      });
    }
  }
  ...
  function onMouseUp(e) {
    ...
    if (lineStartNode) {
      ...
      if (targetNode) {
        setConnections((prev) => [
          ...prev,
          { from: lineStartNode, to: targetNode.id, label: "" },
        ]);
      }
  ...
  function onWheel(e) {
    ...
    setZoom((prev) => Math.max(0.3, Math.min(2.5, prev * scaleAmount)));
  }
  ...
  function onMiddleMouseDown(e) {
    e.preventDefault();
    setPanning(true);
    setPanStart({ x: pan.x, y: pan.y, startX: e.clientX, startY: e.clientY });
  }
  ...
  const handleUpdateConnection = (updatedConnection) => {
    setConnections((prevConnections) =>
      prevConnections.map((conn) =>
        conn.from === updatedConnection.from && conn.to === updatedConnection.to
          ? updatedConnection
          : conn
      )
    );
    ...
    <div className="flex gap-4 select-none">
      ...
      >
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="text-sm text-gray-500">Drag, connect, pan, zoom</div>
        </div>
        ...
        >
          {/* --- Grid --- */}
          <div
            className="absolute inset-0 bg-[repeating-linear-gradient(0deg,#e6e6e6_0_1px,transparent_1px,transparent_20px),repeating-linear-gradient(90deg,#e6e6e6_0_1px,transparent_1px,transparent_20px)] opacity-30 pointer-events-none"
            style={{
          ...
          >
            {nodes.map((n) => (
              <NodeCard
                ...
                onStartConnection={() => setLineStartNode(n.id)}
